...
def test_default_project(cookies):
    ...
    try:
        ...
        subprocess.run(
            ["docker", "run", "--rm", "zenable-io/todo:latest", "--help"],
            capture_output=True,
            check=True,
            cwd=project,
        )
        ...
        with open(project.joinpath(".github/project.yml"), encoding="utf-8") as yaml_data:
    ...
    for filename in ["commit.yml"]:
        with open(project.joinpath(f".github/workflows/{filename}"), encoding="utf-8") as file:
            try:
