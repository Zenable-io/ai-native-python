...
def test_project_tasks():
    ...
    try:
        ...
        sbom_files = list(project_root.glob("sbom.*.json"))
        assert len(sbom_files) >= 3, f"Expected at least 3 SBOM files, found {len(sbom_files)}"
        ...
        vuln_files = list(project_root.glob("vulns.*.json"))
        assert len(vuln_files) >= 1, f"Expected at least 1 vulnerability scan file, found {len(vuln_files)}"
        ...
        license_files = list(project_root.glob("license-check.*.json"))
        assert len(license_files) >= 1, f"Expected at least 1 license check file, found {len(license_files)}"
...
def test_docker_image():
    ...
    try:
        ...
        # Test basic docker run without arguments (should show help or run default)
        ...
        subprocess.run(
            ["docker", "run", "--rm", "-e", "LOG_LEVEL=DEBUG", f"{image_name}:latest", "--help"],
            capture_output=True,
            check=True,
            cwd=project_root,
        )
...
def test_multi_platform_build():
    ...
    try:
        ...
        for platform in ["linux/amd64", "linux/arm64"]:
            ...
            sbom_file = project_root / f"sbom.{platform_suffix}.syft.json"
            ...
            vuln_file = project_root / f"vulns.{platform_suffix}.json"
            assert vuln_file.exists(), f"Vulnerability scan file for {platform} not found"
