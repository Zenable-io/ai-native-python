...
class InfrastructureDependency(BaseModel):
    ...
    version: Optional[str] = Field(None, pattern=r"^(0|[1-9]\d*)\.(0|[1-9]\d*)(\.(0|[1-9]\d*))?$")
...
class DependsOnConfig(BaseModel):
    ...
    infrastructure: list[InfrastructureDependency] = Field(
        default_factory=list, description="A list of infrastructure that this component depends on"
    )
    service: list[ServiceDependency] = Field(
        default_factory=list, description="A list of services that this component depends on"
    )
...
class LanguageConfig(BaseModel):
    ...
    name: Literal["python", "go", "javascript", "typescript", "java", "csharp", "rust", "ruby", "php"]
...
class DevelopConfig(BaseModel):
    ...
    depends_on: Optional[DependsOnConfig] = Field(
        default_factory=DependsOnConfig, description="The dependencies of this service during development"
    )
...
class DeployConfig(BaseModel):
    ...
    deploy_type: Literal["sam", "opentofu", "cloudformation", "kubernetes", "helm", "terraform", "ansible", "none"]
    depends_on: Optional[DependsOnConfig] = Field(
        default_factory=DependsOnConfig, description="The dependencies of this service in order to deploy"
    )
    environments: list[Literal["development", "testing", "staging", "production", "sandbox", "qa", "uat", "demo", "dr"]] = Field(
        default_factory=list, description="A list of environments that this service can run in"
    )
...
class RuntimeConfig(BaseModel):
    ...
    protocols: list[Literal["http", "https", "ws", "wss", "grpc", "graphql", "rest", "soap", "amqp", "mqtt"]] = Field(
        default_factory=list, description="The protocols that the service supports"
    )
    environments: dict[
        Literal["development", "testing", "staging", "production", "sandbox", "qa", "uat", "demo", "dr"],
    ...
    runtime_type: Literal[
        "compose", "container", "serverless", "lambda", "function", "vm", "bare_metal", "kubernetes", "process", "batch", "cron", "none"
    ]
    platforms: list[Literal["linux/amd64", "linux/arm64"]] = Field(
        default_factory=list, description="A list of platforms that this service can run on"
    )
...
class ServiceSchema(BaseModel):
    ...
    def check_lifecycle_constraints(self):
        ...
        production_restricted_lifecycles = ["experimental", "prototype", "development", "deprecated", "sunset", "retired", "archived"]

        if self.deploy and self.deploy.environments:
            if lifecycle in production_restricted_lifecycles and "production" in self.deploy.environments:
                raise ValueError(
